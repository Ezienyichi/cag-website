...
export default function VerifyPage() {
  ...
  useEffect(() => {
    ...
    if (trxStatus === 'successful' && transactionId) {
      ...
      // Webhook will handle it — assume success if tx_ref exists
      if (txRef) {
        setStatus('success');
        clearCart();
      } else {
      ...
      <main className="pt-28 pb-16 px-6 md:px-8 max-w-screen-xl mx-auto min-h-screen flex items-center justify-center">
        ...
        {status === 'failed' && (
          <div className="text-center max-w-lg">
            ...
            <p className="text-on-surface-variant text-lg mb-8">
              Your payment could not be verified. If money was deducted, please contact us
              and we&apos;ll resolve it promptly.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href="/cart"
                className="bg-primary-container text-on-primary-container px-8 py-3 rounded-full font-bold font-headline"
              >
                Try Again
              </Link>
              <a
                href="mailto:hello@changeartgallerie.com"
                className="bg-surface-container-low text-primary px-8 py-3 rounded-full font-bold font-headline"
              >
                Contact Support
              </a>
            </div>
